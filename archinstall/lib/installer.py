...
class Installer:
    def __init__(
        ...
        self._disk_encryption = disk_encryption or disk.DiskEncryption(
            disk.EncryptionType.NoEncryption
        )
    ...
    def __exit__(
        ...
        if exc_type is not None:
            ...
            self.copy_skel_to_user_home(self.user)

            # We avoid printing /mnt/<log path> because that might confuse people if they note it down
            # and then reboot, and a identical log file will be found in the ISO medium anyway.
            log_file = os.path.join(storage["LOG_PATH"], storage["LOG_FILE"])
            Tui.print(
                str(_("[!] A log file has been created here: {}").format(log_file))
            )
            Tui.print(
                str(
                    _(
    ...
    def _verify_service_stop(self) -> None:
        ...
        if not storage["arguments"].get("skip_ntp", False):
            ...
            info(
                _(
                    "Skipping waiting for automatic time sync (this can cause issues if time is out of sync during installation)"
                )
            )
        ...
        while self._service_state("archlinux-keyring-wkd-sync.service") not in (
            "dead",
            "failed",
            "exited",
        ):
    ...
    def _verify_boot_part(self) -> None:
        ...
        if len(lsblk_info) > 0:
            if lsblk_info[0].size < disk.Size(
                200, disk.Unit.MiB, disk.SectorSize.default()
            ):
                raise DiskError(
    ...
    def mount_ordered_layout(self) -> None:
        ...
        match self._disk_encryption.encryption_type:
            ...
            case disk.EncryptionType.Luks:
                luks_handlers = self._prepare_luks_partitions(
                    self._disk_encryption.partitions
                )
            ...
            case disk.EncryptionType.LuksOnLvm:
                self._import_lvm()
                luks_handlers = self._prepare_luks_lvm(
                    self._disk_encryption.lvm_volumes
                )
                self._mount_lvm_layout(luks_handlers)
    ...
    def _mount_partition_layout(self, luks_handlers: dict[Any, Luks2]) -> None:
        ...
        for mod in sorted_device_mods:
            ...
            # '/' just for sorting
            sorted_part_mods = sorted(
                not_pv_part_mods, key=lambda x: x.mountpoint or Path("/")
            )
    ...
    def _prepare_luks_partitions(
        self, partitions: list[disk.PartitionModification]
    ...
    def _prepare_luks_lvm(
        self, lvm_volumes: list[disk.LvmVolume]
    ) -> dict[disk.LvmVolume, Luks2]:
        return {
            vol: disk.device_handler.unlock_luks2_dev(
                vol.dev_path, vol.mapper_name, self._disk_encryption.encryption_password
            )
    ...
    def _mount_partition(self, part_mod: disk.PartitionModification) -> None:
        ...
        if part_mod.mountpoint and part_mod.dev_path:
            ...
            disk.device_handler.mount(
                part_mod.dev_path, target, options=part_mod.mount_options
            )

        if part_mod.fs_type == disk.FilesystemType.Btrfs and part_mod.dev_path:
            self._mount_btrfs_subvol(
                part_mod.dev_path, part_mod.btrfs_subvols, part_mod.mount_options
            )
    ...
    def _mount_lvm_vol(self, volume: disk.LvmVolume) -> None:
        if volume.fs_type != disk.FilesystemType.Btrfs:
            if volume.mountpoint and volume.dev_path:
                ...
                disk.device_handler.mount(
                    volume.dev_path, target, options=volume.mount_options
                )

        if volume.fs_type == disk.FilesystemType.Btrfs and volume.dev_path:
            self._mount_btrfs_subvol(
                volume.dev_path, volume.btrfs_subvols, volume.mount_options
            )

    def _mount_luks_partition(
        self, part_mod: disk.PartitionModification, luks_handler: Luks2
    ) -> None:
        ...
        if part_mod.fs_type == disk.FilesystemType.Btrfs and part_mod.btrfs_subvols:
            self._mount_btrfs_subvol(
                luks_handler.mapper_dev, part_mod.btrfs_subvols, part_mod.mount_options
            )
        elif part_mod.mountpoint:
            target = self.target / part_mod.relative_mountpoint
            disk.device_handler.mount(
                luks_handler.mapper_dev, target, options=part_mod.mount_options
            )
    ...
    def _mount_luks_volume(self, volume: disk.LvmVolume, luks_handler: Luks2) -> None:
        if volume.fs_type != disk.FilesystemType.Btrfs:
            if volume.mountpoint and luks_handler.mapper_dev:
                target = self.target / volume.relative_mountpoint
                disk.device_handler.mount(
                    luks_handler.mapper_dev, target, options=volume.mount_options
                )

        if volume.fs_type == disk.FilesystemType.Btrfs and luks_handler.mapper_dev:
            self._mount_btrfs_subvol(
                luks_handler.mapper_dev, volume.btrfs_subvols, volume.mount_options
            )
    ...
    def _generate_key_files_partitions(self) -> None:
        for part_mod in self._disk_encryption.partitions:
            gen_enc_file = self._disk_encryption.should_generate_encryption_file(
                part_mod
            )
    ...
    def add_swapfile(
        self, size: str = "4G", enable_resume: bool = True, file: str = "/swapfile"
    ) -> None:
        if file[:1] != "/":
            file = f"/{file}"
        if len(file.strip()) <= 0 or file == "/":
            raise ValueError(
                f"The filename for the swap file has to be a valid path, not: {self.target}{file}"
            )
        ...
        if enable_resume:
            resume_uuid = SysCommand(
                f"findmnt -no UUID -T {self.target}{file}"
            ).decode()
            resume_offset = (
                SysCommand(f"/usr/bin/filefrag -v {self.target}{file}")
                .decode()
                .split("0:", 1)[1]
                .split(":", 1)[1]
                .split("..", 1)[0]
    ...
    def set_mirrors(
        self, mirror_config: MirrorConfiguration, on_target: bool = False
    ) -> None:
        """
    ...
    def set_locale(self, locale_config: LocaleConfiguration) -> bool:
        ...
        if "." in locale_config.sys_lang:
            ...
            if (
                ...
                and locale_config.sys_enc != potential_encoding
            ):
        ...
        for index, line in enumerate(locale_gen_lines):
            ...
            error(
                f"Invalid locale: language '{locale_config.sys_lang}', encoding '{locale_config.sys_enc}'"
            )
            return False
            ...
            if hasattr(plugin, "on_configure_nic"):
                conf = (
                    plugin.on_configure_nic(
                        nic.iface, nic.dhcp, nic.ip, nic.gateway, nic.dns
                    )
                    or conf
                )

        with open(
            f"{self.target}/etc/systemd/network/10-{nic.iface}.network", "a"
        ) as netconf:
    ...
    def copy_iso_network_config(self, enable_services: bool = False) -> bool:
            ...
            if psk_files := glob.glob("/var/lib/iwd/*.psk"):
                ...
                if enable_services:
                    ...
                    if self.helper_flags.get("base", False) is False:
                        ...
                        def post_install_enable_iwd_service(
                            *args: str, **kwargs: str
                        ) -> None:
        ...
        if netconfigurations := glob.glob("/etc/systemd/network/*"):
            ...
            for netconf_file in netconfigurations:
                shutil.copy2(
                    netconf_file,
                    f"{self.target}/etc/systemd/network/{os.path.basename(netconf_file)}",
                )
            ...
            if enable_services:
                ...
                if self.helper_flags.get("base", False) is False:
                    ...
                    def post_install_enable_networkd_resolved(
                        *args: str, **kwargs: str
                    ) -> None:
    ...
    def mkinitcpio(self, flags: list[str]) -> bool:
        ...
        with open(f"{self.target}/etc/mkinitcpio.conf", "r+") as mkinit:
            ...
            if not self._disk_encryption.hsm_device:
                ...
                # * sd-vconsole -> keymap
                self._hooks = [
                    hook.replace("systemd", "udev").replace(
    ...
    def _handle_partition_installation(self) -> None:
        ...
        for mod in self._disk_config.device_modifications:
            for part in mod.partitions:
                ...
                if (
                    part.fs_type.fs_type_mount == "ntfs3"
                    and part.mountpoint == self.target
                ):
                ...
                if part in self._disk_encryption.partitions:
                    if self._disk_encryption.hsm_device:
                        ...
                        if "sd-encrypt" not in self._hooks:
                            self._hooks.insert(
                                self._hooks.index("filesystems"), "sd-encrypt"
                            )
                    else:
                        if "encrypt" not in self._hooks:
                            self._hooks.insert(
                                self._hooks.index("filesystems"), "encrypt"
                            )
        ...
        for vg in self._disk_config.lvm_config.vol_groups:
            for vol in vg.volumes:
                if vol.fs_type is not None:
                    ...
                    # There is not yet an fsck tool for NTFS. If it's being used for the root filesystem, the hook should be removed.
                    if (
                        vol.fs_type.fs_type_mount == "ntfs3"
                        and vol.mountpoint == self.target
                    ):
                        if "fsck" in self._hooks:
                            self._hooks.remove("fsck")

        if self._disk_encryption.encryption_type in [
            disk.EncryptionType.LvmOnLuks,
            disk.EncryptionType.LuksOnLvm,
        ]:
            if self._disk_encryption.hsm_device:
                # Required by mkinitcpio to add support for fido2-device options
                self.pacman.strap("libfido2")
    ...
    def minimal_installation(
        ...
        if testing:
            info(
                "The testing flag is set. This system will be installed with testing repositories enabled."
            )
    ...
    def setup_swap(self, kind: str = "zram") -> None:
        if kind == "zram":
            ...
            with open(
                f"{self.target}/etc/systemd/zram-generator.conf", "w"
            ) as zram_conf:
            ...
            raise ValueError(
                "Archinstall currently only supports setting up swap on zram"
            )
    ...
    def _get_luks_uuid_from_mapper_dev(self, mapper_dev_path: Path) -> str:
        lsblk_info = disk.get_lsblk_info(
            mapper_dev_path, reverse=True, full_dev_path=True
        )
    ...
    def _get_kernel_params_partition(
        ...
        if root_partition in self._disk_encryption.partitions:
            ...
            if self._disk_encryption and self._disk_encryption.hsm_device:
                debug(
                    f"Root partition is an encrypted device, identifying by UUID: {root_partition.uuid}"
                )
                ...
                kernel_parameters.append(
                    "rd.luks.options=fido2-device=auto,password-echo=no"
                )
            elif partuuid:
                debug(
                    f"Root partition is an encrypted device, identifying by PARTUUID: {root_partition.partuuid}"
                )
                kernel_parameters.append(
                    f"cryptdevice=PARTUUID={root_partition.partuuid}:root"
                )
            else:
                debug(
                    f"Root partition is an encrypted device, identifying by UUID: {root_partition.uuid}"
            ...
            if partuuid:
                debug(
                    f"Identifying root partition by PARTUUID: {root_partition.partuuid}"
                )
    ...
    def _get_kernel_params_lvm(self, lvm: disk.LvmVolume) -> list[str]:
        kernel_parameters = []
        ...
        match self._disk_encryption.encryption_type:
            case disk.EncryptionType.LvmOnLuks:
                ...
                if not pv_seg_info:
                    raise ValueError(
                        f"Unable to determine PV segment info for {lvm.vg_name}/{lvm.name}"
                    )
                ...
                if self._disk_encryption.hsm_device:
                    debug(
                        f"LvmOnLuks, encrypted root partition, HSM, identifying by UUID: {uuid}"
                    )
                    kernel_parameters.append(
                        f"rd.luks.name={uuid}=cryptlvm root={lvm.safe_dev_path}"
                    )
                else:
                    debug(
                        f"LvmOnLuks, encrypted root partition, identifying by UUID: {uuid}"
                    )
                    kernel_parameters.append(
                        f"cryptdevice=UUID={uuid}:cryptlvm root={lvm.safe_dev_path}"
                    )
            case disk.EncryptionType.LuksOnLvm:
                ...
                if self._disk_encryption.hsm_device:
                    debug(
                        f"LuksOnLvm, encrypted root partition, HSM, identifying by UUID: {uuid}"
                    )
                    kernel_parameters.append(
                        f"rd.luks.name={uuid}=root root=/dev/mapper/root"
                    )
                else:
                    debug(
                        f"LuksOnLvm, encrypted root partition, identifying by UUID: {uuid}"
                    )
                    kernel_parameters.append(
                        f"cryptdevice=UUID={uuid}:root root=/dev/mapper/root"
                    )
            case disk.EncryptionType.NoEncryption:
    ...
    def _get_kernel_params(
        ...
        if isinstance(root, disk.LvmVolume):
            ...
            kernel_parameters = self._get_kernel_params_partition(
                root, id_root, partuuid
            )
    ...
    def _add_systemd_bootloader(
        ...
        try:
            SysCommand(
                f"/usr/bin/arch-chroot {self.target} bootctl {' '.join(bootctl_options)} install"
            )
        except SysCallError:
            # Fallback, try creating the boot loader without touching the EFI variables
            SysCommand(
                f"/usr/bin/arch-chroot {self.target} bootctl --no-variables {' '.join(bootctl_options)} install"
            )
        ...
        try:
            ...
            loader_data = [default, "timeout 15"]
        else:
        ...
        comments = ("# Created by: archinstall", f"# Created on: {self.init_time}")
    ...
    def _add_grub_bootloader(
        ...
        command = ["/usr/bin/arch-chroot", str(self.target), "grub-install", "--debug"]
        ...
        if SysInfo.has_uefi():
            ...
            try:
                ...
                try:
                    ...
                    raise DiskError(
                        f"Could not install GRUB to {self.target}{efi_partition.mountpoint}: {err}"
                    )
        else:
            info(f"GRUB boot partition: {boot_partition.dev_path}")

            parent_dev_path = disk.device_handler.get_parent_device_path(
                boot_partition.safe_dev_path
            )

            add_options = ["--target=i386-pc", "--recheck", str(parent_dev_path)]

            try:
                SysCommand(command + add_options, peek_output=True)
            except SysCallError as err:
                raise DiskError(
                    f"Failed to install GRUB boot on {boot_partition.dev_path}: {err}"
                )
    ...
    def _add_limine_bootloader(
        ...
        if SysInfo.has_uefi():
            ...
            try:
                efi_dir_path = (
                    self.target
                    / efi_partition.mountpoint.relative_to("/")
                    / "EFI"
                    / "BOOT"
                )
                ...
                raise DiskError(
                    f"Failed to install Limine in {self.target}{efi_partition.mountpoint}: {err}"
                )
            ...
            if unique_path := disk.device_handler.get_unique_path_for_device(
                parent_dev_path
            ):
...
"""
    ...
    def _add_efistub_bootloader(
        ...
        if not uki_enabled:
            ...
            entries = ("initrd=/initramfs-{kernel}.img", *self._get_kernel_params(root))
        ...
        * efistub (beta)
        ...
        match bootloader:
            case Bootloader.Systemd:
                self._add_systemd_bootloader(
                    boot_partition, root, efi_partition, uki_enabled
                )
    ...
    def enable_sudo(self, entity: str, group: bool = False):
        ...
        file_num_str = "{:02d}".format(
            num_of_rules_already
        )  # We want 00_user1, 01_user2, etc
    ...
    def user_set_pw(self, user: str, password: str) -> bool:
        ...
        try:
            SysCommand(
                f"/usr/bin/arch-chroot {self.target} " + sh[:-1] + " | chpasswd'"
            )
            return True
        except SysCallError:
            return False
    ...
    def chown(self, owner: str, path: str, options: list[str] = []) -> bool:
        ...
        try:
            SysCommand(
                f"/usr/bin/arch-chroot {self.target} sh -c 'chown {' '.join(options)} {owner} {cleaned_path}'"
            )
            return True
        except SysCallError:
            return False
    ...
    def set_keyboard_language(self, language: str) -> bool:
        ...
        if len(language.strip()):
            ...
            with Boot(self) as session:
                os.system(
                    '/usr/bin/systemd-run --machine=archinstall --pty localectl set-keymap ""'
                )
                ...
                try:
                    ...
                    raise ServiceException(
                        f"Unable to set locale '{language}' for console: {err}"
                    )
        ...
        if len(language.strip()):
            ...
            with Boot(self) as session:
                ...
                try:
                    ...
                    raise ServiceException(
                        f"Unable to set locale '{language}' for X11: {err}"
                    )
        else:
            info(
    ...
    def copy_skel_to_user_home(self, user: str) -> str:
        """
        Copies the content of /etc/skel to the home directory of the specified user.

        Parameters:
                user (str): The username of the user whose home directory will be updated.
        ...
        if not os.path.exists(home_path):
            return f"Error: Home directory for user '{user}' does not exist."
